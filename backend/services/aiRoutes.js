...
const sentenceFromSource = (source = '') => String(source || '')
  .replace(/\s+/g, ' ')
  .split(/(?<=[.!?])\s+/)
  .map((sentence) => sentence.trim())
  .find((sentence) => sentence.length >= 80)
  || truncate(String(source || '').replace(/\s+/g, ' ').trim(), 220);

const fallbackQuizFromSource = (source = '') => {
  const sentence = sentenceFromSource(source);
...
const fallbackFlashcardsFromSource = (source = '') => {
  const sentence = sentenceFromSource(source);
  if (!sentence) return null;
  return {
    type: 'flashcards',
...
Create 5 questions. Keep options clear, plausible, and exam-focused.`;
        break;
      case 'flashcards':
...
Then add a short note on what to revise first.`;
    ...
    if (hasDocumentContext) {
      ...
      if (cached) {
        const structuredRequired = ['quiz', 'generate-quiz', 'flashcards', 'generate-flashcards'].includes(action);
        if (structuredRequired && !cached.structured) {
          await AiGenerationCacheService.delete?.(generationCacheKey);
        } else {
        return res.json({
...
${structuredInstructionForAction(action)}`;
    ...
    if (!structured && ['quiz', 'generate-quiz'].includes(action)) {
      structured = fallbackQuizFromSource(sourceTextForMemory || userPrompt);
    }
    if (!structured && ['flashcards', 'generate-flashcards'].includes(action)) {
      structured = fallbackFlashcardsFromSource(sourceTextForMemory || userPrompt);
    }
